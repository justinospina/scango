...
class _ScanGoAppState extends State<ScanGoApp> with WidgetsBindingObserver {
  ...
  Future<void> _solicitarBiometria() async {
    ...
    try {
      ...
      final exitoso = await _auth.authenticate(localizedReason: 'Desbloquea ScanGo para continuar');
      if (exitoso) {
        final miId = Supabase.instance.client.auth.currentUser?.id;
        if (miId != null) await Supabase.instance.client.from('perfiles').update({'verificado_biometria': true}).eq('id', miId);
      }
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: Supabase.instance.client.auth.currentSession == null
          ...
          : (_autenticado ? const PantallaPrincipal() : PantallaBloqueo(onReintentar: _solicitarBiometria)),
    );
  }
}
        ...
        child: Column(
          ...
          children: [
            ...
            ElevatedButton.icon(
              ...
              style: ElevatedButton.styleFrom(backgroundColor: Colors.greenAccent, foregroundColor: Colors.black, minimumSize: const Size(200, 50)),
            )
          ],
        ),
      ),
...
class ModalPerfilDetalle extends StatefulWidget {
  ...
  const ModalPerfilDetalle({super.key, required this.perfil, required this.distanciaTxt, this.relacionExistenteInit, required this.miId});
  @override
...
class _ModalPerfilDetalleState extends State<ModalPerfilDetalle> {
  ...
  Future<void> _alternarLike() async {
    ...
    try {
      if (_relacion == null) {
        final res = await Supabase.instance.client.from('solicitudes').insert({'emisor_id': widget.miId, 'receptor_id': otroId, 'estado': 'pendiente', 'emisor_like': true}).select().single();
...
class PantallaPrincipalState extends State<PantallaPrincipal> {
  ...
  void _preguntarDeseoDeHoy() {
    ...
    final txtDeseoController = TextEditingController();
      ...
      builder: (context) {
        return AlertDialog(
          ...
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              const Text('Escribe libremente qué buscas hacer hoy.', style: TextStyle(color: Colors.grey, fontSize: 13)),
              const SizedBox(height: 20),
              TextField(
                controller: txtDeseoController,
                textCapitalization: TextCapitalization.sentences,
                decoration: InputDecoration(
                  hintText: 'Ej. Tomar un café, entrenar, conversar...',
          ...
          actions: [
            ...
            ElevatedButton(
              style: ElevatedButton.styleFrom(backgroundColor: Colors.greenAccent, foregroundColor: Colors.black),
              onPressed: () async {
                final nuevoDeseo = txtDeseoController.text.trim();
  ...
  Future<void> _actualizarUltimaConexion() async {
    final miId = Supabase.instance.client.auth.currentUser?.id;
    if (miId != null) {
      await Supabase.instance.client.from('perfiles').update({'ultima_conexion': DateTime.now().toUtc().toIso8601String()}).eq('id', miId);
    }
  }
  ...
  Future<void> _iniciarRastreoGPS() async {
    try {
      ...
      if (mounted) setState(() { _miLatitud = posInicial.latitude; _miLongitud = posInicial.longitude; });

      const LocationSettings locationSettings = LocationSettings(accuracy: LocationAccuracy.high, distanceFilter: 15);
      _positionStream = Geolocator.getPositionStream(locationSettings: locationSettings).listen((Position position) async {
        if (mounted) setState(() { _miLatitud = position.latitude; _miLongitud = position.longitude; });
        final miId = Supabase.instance.client.auth.currentUser?.id;
        if (miId != null) {
          await Supabase.instance.client.from('perfiles').update({'latitud': position.latitude, 'longitud': position.longitude, 'ultima_conexion': DateTime.now().toUtc().toIso8601String()}).eq('id', miId);
        }
      });
    } catch (e) {
  ...
  void _actualizarBadgeDelIcono(int contador) {
    if (!kIsWeb) {
      FlutterAppBadger.isAppBadgeSupported().then((soportado) {
        if (soportado) {
          if (contador > 0) FlutterAppBadger.updateBadgeCount(contador);
          else FlutterAppBadger.removeBadge();
        }
      });
    }
  }
        ...
        actions: [
            ...
            children: [
              ...
              Switch(
                ...
                onChanged: (val) async {
                  ...
                  if (miId != null) await Supabase.instance.client.from('perfiles').update({'disponible': val}).eq('id', miId);
                },
              ),
            ],
          ),
          IconButton(
            ...
            onPressed: () async {
              ...
              if (context.mounted) Navigator.of(context).pushAndRemoveUntil(MaterialPageRoute(builder: (_) => const PantallaMuro(esInvitado: true)), (route) => false);
            },
          )
        ],
      ),
              ...
              if (miId != null) {
                ...
                if (snapshotMensajes.hasData && snapshotSolicitudes.hasData) {
                  final chatsActivosIds = snapshotSolicitudes.data!
                      ...
                      .map((s) => s['emisor_id'] == miId ? s['receptor_id'] : s['emisor_id']).toSet();
                  mensajesNoLeidos = snapshotMensajes.data!
                      .where((m) => m['receptor_id'] == miId && chatsActivosIds.contains(m['emisor_id']) && (m['leido'] == null || m['leido'] == false)).length;
                }
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                _procesando ? const CircularProgressIndicator() : Column(
                  children: [
  ...
  void _mostrarPoliticas() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [TextButton(onPressed: () => Navigator.pop(context), child: const Text('Entendido', style: TextStyle(color: Colors.greenAccent)))],
      ),
    );
  }
            ...
            child: Column(
              ...
              children: [
                ...
                if (_generoDetectado != null && !_procesandoIA) ...[
                  DropdownButtonFormField<String>(value: _generoDetectado, decoration: const InputDecoration(labelText: 'Confirma tu Género', border: OutlineInputBorder()), items: ['MUJER', 'HOMBRE'].map((label) => DropdownMenuItem(value: label, child: Text(label))).toList(), onChanged: (value) => setState(() => _generoDetectado = value!)),
                  const SizedBox(height: 15),
                  ...
                  const SizedBox(height: 15),
                  DropdownButtonFormField<String>(value: _preferencia, decoration: const InputDecoration(labelText: 'Preferencia', border: OutlineInputBorder()), items: ['MUJER', 'HOMBRE', 'AMBAS'].map((label) => DropdownMenuItem(value: label, child: Text(label))).toList(), onChanged: (value) => setState(() => _preferencia = value!)),
                  const SizedBox(height: 15),
                  Row(
                    children: [
                      Checkbox(value: _aceptaPoliticas, activeColor: Colors.greenAccent, checkColor: Colors.black, onChanged: (val) => setState(() => _aceptaPoliticas = val ?? false)),
                      Expanded(child: GestureDetector(onTap: _mostrarPoliticas, child: const Text('Acepto la Política de Tratamiento de Datos Personales', style: TextStyle(color: Colors.blueAccent, decoration: TextDecoration.underline, fontSize: 13))))
                    ],
                  ),
                  const SizedBox(height: 20),
                  _procesando ? const CircularProgressIndicator() : ElevatedButton(onPressed: registrarYGuardar, style: ElevatedButton.styleFrom(backgroundColor: Colors.greenAccent, foregroundColor: Colors.black, minimumSize: const Size(double.infinity, 50)), child: const Text('Completar Registro'))
                ]
              ],
            ),
          ),
...
class _PantallaMuroState extends State<PantallaMuro> {
  ...
  Future<void> _cargarPublicaciones() async {
    ...
    try {
      ...
      if (mounted) setState(() { _publicaciones = List<Map<String, dynamic>>.from(data); _hayMas = data.length == _limitePagina; });
    } catch (e) {
  ...
  Widget _construirBuscadorDinamico({required String label, required Iterable<String> opciones, required Function(String) onSelected, required VoidCallback onCleared}) {
    return Autocomplete<String>(
      ...
      fieldViewBuilder: (context, controller, focusNode, onFieldSubmitted) {
        return TextField(
          ...
          decoration: InputDecoration(labelText: label, isDense: true, border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)), suffixIcon: IconButton(icon: const Icon(Icons.clear, size: 18), onPressed: () { controller.clear(); onCleared(); })),
        );
      },
    );
  }
  ...
  Future<void> _eliminarPublicacion(String pubId) async {
    final confirmar = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        backgroundColor: Colors.grey[900], title: const Text('¿Eliminar publicación?', style: TextStyle(color: Colors.redAccent)), content: const Text('Esta acción borrará la publicación del muro de todos los exploradores.'),
  ...
  Future<void> _abrirCrearPublicacion() async {
    ...
    XFile? mediaFile; String? mediaTipo; bool procesando = false;
    String? depSel; String? ciuSel;

    await showModalBottomSheet(
      context: context, isScrollControlled: true, backgroundColor: Colors.grey[900], shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(20))),
      builder: (context) {
        return StatefulBuilder(
          builder: (context, setStateModal) {
            Future<void> publicar() async {
              final texto = txtController.text.trim();
              if ((texto.isEmpty && mediaFile == null) || depSel == null || ciuSel == null) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Agrega contenido, departamento y ciudad para publicar'))); return; }
              setStateModal(() => procesando = true);
              String? urlFinal;
              try {
                if (mediaFile != null) {
                  final fileName = '${miId}_${DateTime.now().millisecondsSinceEpoch}.${mediaTipo == 'vid' ? 'mp4' : 'jpg'}';
                  if (!kIsWeb) { await Supabase.instance.client.storage.from('chat-media').upload(fileName, File(mediaFile!.path)); } 
                  else { await Supabase.instance.client.storage.from('chat-media').uploadBinary(fileName, await mediaFile!.readAsBytes()); }
                  urlFinal = Supabase.instance.client.storage.from('chat-media').getPublicUrl(fileName);
                }
                await Supabase.instance.client.from('publicaciones').insert({'usuario_id': miId, 'texto': texto, 'media_url': urlFinal, 'tipo': mediaTipo, 'whatsapp': whatsappController.text.trim(), 'departamento': depSel, 'ciudad': ciuSel});
                if (context.mounted) { Navigator.pop(context); ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('✅ Publicado con éxito'), backgroundColor: Colors.green)); setState(() => _paginaActual = 0); _cargarPublicaciones(); }
              } catch (e) {
              ...
              child: ConstrainedBox(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        Expanded(child: _construirBuscadorDinamico(label: 'Departamento', opciones: ColombiaData.ubicaciones.keys, onSelected: (val) => setStateModal(() { depSel = val; ciuSel = null; }), onCleared: () => setStateModal(() { depSel = null; ciuSel = null; }))),
                        const SizedBox(width: 10),
                        Expanded(child: _construirBuscadorDinamico(label: 'Ciudad', opciones: depSel == null ? [] : ColombiaData.ubicaciones[depSel]!, onSelected: (val) => setStateModal(() => ciuSel = val), onCleared: () => setStateModal(() => ciuSel = null))),
                      ],
                    ),
                    const SizedBox(height: 10),
                    TextField(controller: txtController, maxLines: 2, decoration: InputDecoration(hintText: '¿Qué quieres compartir?', border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)))),
                    const SizedBox(height: 10),
                    TextField(controller: whatsappController, keyboardType: TextInputType.phone, decoration: InputDecoration(hintText: 'Tu WhatsApp (Opcional)', prefixIcon: const Icon(Icons.phone, color: Colors.greenAccent), border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)), isDense: true)),
                    const SizedBox(height: 10),
                    if (mediaFile != null)
                      Container(
                        padding: const EdgeInsets.all(8), margin: const EdgeInsets.only(bottom: 10), decoration: BoxDecoration(color: Colors.grey[800], borderRadius: BorderRadius.circular(8)),
                        child: Row(children: [Icon(mediaTipo == 'vid' ? Icons.videocam : Icons.image, color: Colors.greenAccent), const SizedBox(width: 10), Expanded(child: Text(mediaFile!.name, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 12))), IconButton(icon: const Icon(Icons.close, color: Colors.red), onPressed: () => setStateModal(() { mediaFile = null; mediaTipo = null; }))]),
                      ),
                    Row(
                      mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                      children: [
                        IconButton(icon: const Icon(Icons.camera_alt), color: Colors.blueAccent, onPressed: () async { final f = await _picker.pickImage(source: ImageSource.camera, imageQuality: 70); if (f != null) setStateModal(() { mediaFile = f; mediaTipo = 'img'; }); }),
                        IconButton(icon: const Icon(Icons.image), color: Colors.blueAccent, onPressed: () async { final f = await _picker.pickImage(source: ImageSource.gallery, imageQuality: 70); if (f != null) setStateModal(() { mediaFile = f; mediaTipo = 'img'; }); }),
                        IconButton(icon: const Icon(Icons.videocam), color: Colors.blueAccent, onPressed: () async { final f = await _picker.pickVideo(source: ImageSource.camera); if (f != null) setStateModal(() { mediaFile = f; mediaTipo = 'vid'; }); }),
                      ],
                    ),
                    const SizedBox(height: 10),
                    procesando ? const CircularProgressIndicator() : ElevatedButton(onPressed: publicar, style: ElevatedButton.styleFrom(backgroundColor: Colors.greenAccent, foregroundColor: Colors.black, minimumSize: const Size(double.infinity, 50)), child: const Text('Publicar Ahora')),
                    const SizedBox(height: 20),
                  ],
                ),
              ),
  ...
  Widget build(BuildContext context) {
    final miId = Supabase.instance.client.auth.currentUser?.id;
    return Scaffold(
      appBar: widget.esInvitado ? AppBar(title: const Text('ScanGo', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 24)), backgroundColor: Colors.grey[900], actions: [TextButton.icon(onPressed: () => Navigator.of(context).push(MaterialPageRoute(builder: (_) => const PantallaLogin())), icon: const Icon(Icons.login, color: Colors.greenAccent), label: const Text('Ingresar', style: TextStyle(color: Colors.greenAccent, fontWeight: FontWeight.bold)))]) : null,
      body: Center(
        child: ConstrainedBox(
          constraints: const BoxConstraints(maxWidth: 700),
          child: Column(
            children: [
              Container(
                padding: const EdgeInsets.all(12), color: Colors.grey[850],
                child: Column(
                  children: [
                    Row(
                      children: [
                        Expanded(child: _construirBuscadorDinamico(label: 'Filtrar Depto', opciones: ColombiaData.ubicaciones.keys, onSelected: (val) { setState(() { _filtroDepartamento = val; _filtroCiudad = null; _paginaActual = 0; }); _cargarPublicaciones(); }, onCleared: () { setState(() { _filtroDepartamento = null; _filtroCiudad = null; _paginaActual = 0; }); _cargarPublicaciones(); })),
                        const SizedBox(width: 10),
                        Expanded(child: _construirBuscadorDinamico(label: 'Filtrar Ciudad', opciones: _filtroDepartamento == null ? [] : ColombiaData.ubicaciones[_filtroDepartamento]!, onSelected: (val) { setState(() { _filtroCiudad = val; _paginaActual = 0; }); _cargarPublicaciones(); }, onCleared: () { setState(() { _filtroCiudad = null; _paginaActual = 0; }); _cargarPublicaciones(); })),
                      ],
                    ),
                  ],
                ),
              ...
              Expanded(
                child: _cargandoMuro ? const Center(child: CircularProgressIndicator()) : _publicaciones.isEmpty ? const Center(child: Text('No hay publicaciones con estos filtros.', style: TextStyle(color: Colors.grey))) : Column(
                  children: [
                    Expanded(
                      child: ListView.builder(
                        padding: const EdgeInsets.all(12), itemCount: _publicaciones.length,
                        itemBuilder: (context, index) {
                          ...
                          if (pub['created_at'] != null) { fecha = DateTime.tryParse(pub['created_at'].toString())?.toLocal() ?? DateTime.now(); }
                          ...
                          return Card(
                            color: Colors.grey[900], margin: const EdgeInsets.only(bottom: 16), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
                            child: Padding(
                                    ...
                                    children: [
                                      ...
                                      Expanded(
                                        child: Column(
                                          crossAxisAlignment: CrossAxisAlignment.start,
                                          children: [
                                            Row(children: [Text(autor['nombre'] ?? 'Usuario', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: Colors.white)), if (esVerificado) ...[const SizedBox(width: 4), const Icon(Icons.verified, color: Colors.blueAccent, size: 16)]]),
                                      ...
                                      if (esMio) IconButton(icon: const Icon(Icons.delete_outline, color: Colors.redAccent), onPressed: () => _eliminarPublicacion(pub['id'].toString())),
                                    ],
                                  ),
                                  const SizedBox(height: 12),
                                  if (pub['texto'] != null && pub['texto'].toString().isNotEmpty) Padding(padding: const EdgeInsets.only(bottom: 12), child: Text(pub['texto'], style: const TextStyle(fontSize: 15))),
                                  if (pub['media_url'] != null) ClipRRect(borderRadius: BorderRadius.circular(12), child: pub['tipo'] == 'vid' ? ReproductorVideoWidget(url: pub['media_url']) : ConstrainedBox(constraints: const BoxConstraints(maxHeight: 250), child: Image.network(pub['media_url'], width: double.infinity, fit: BoxFit.cover, errorBuilder: (_, __, ___) => const Icon(Icons.broken_image, size: 50, color: Colors.grey)))),
                                  if (whatsapp != null && whatsapp.trim().isNotEmpty)
                                    Padding(
                                      padding: const EdgeInsets.only(top: 16),
                                      child: ElevatedButton.icon(
                                        onPressed: () async { final numeroLimpio = whatsapp.replaceAll(RegExp(r'[^0-9]'), ''); final url = Uri.parse('https://wa.me/$numeroLimpio'); if (await canLaunchUrl(url)) { await launchUrl(url, mode: LaunchMode.externalApplication); } else { if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('No se pudo abrir WhatsApp'))); } },
                                        icon: const Icon(Icons.chat, color: Colors.white), label: const Text('Contactar por WhatsApp', style: TextStyle(fontWeight: FontWeight.bold)), style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF25D366), foregroundColor: Colors.white, minimumSize: const Size(double.infinity, 45), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10))),
                                      ),
                                    ),
                                ],
                              ),
                    ...
                    Container(
                      ...
                      child: Row(
                        ...
                        children: [
                          ElevatedButton.icon(onPressed: _paginaActual > 0 ? () { setState(() => _paginaActual--); _cargarPublicaciones(); } : null, icon: const Icon(Icons.arrow_back_ios, size: 14), label: const Text('Anterior')),
                          Text('Página ${_paginaActual + 1}', style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.greenAccent)),
                          ElevatedButton.icon(onPressed: _hayMas ? () { setState(() => _paginaActual++); _cargarPublicaciones(); } : null, icon: const Icon(Icons.arrow_forward_ios, size: 14), label: const Text('Siguiente'), iconAlignment: IconAlignment.end),
                        ],
                      ),
                    )
                  ],
      ...
      floatingActionButton: widget.esInvitado ? null : FloatingActionButton(onPressed: _abrirCrearPublicacion, backgroundColor: Colors.greenAccent, child: const Icon(Icons.add, color: Colors.black)),
    );
  }
}
        ...
        Container(
          ...
          child: Column(
            children: [
              ...
              Slider(value: _distanciaMaximaKm, min: 1, max: 100, divisions: 99, activeColor: Colors.greenAccent, label: '${_distanciaMaximaKm.round()} km', onChanged: (val) => setState(() => _distanciaMaximaKm = val)),
            ],
          ),
        ),
        Expanded(
          child: StreamBuilder<CompassEvent>(
            ...
            builder: (context, snapshotCompass) {
              double heading = snapshotCompass.data?.heading ?? 0;
              double compassRadianes = (heading * math.pi / 180) * -1;
                    ...
                    child: Center(
                      child: Container(
                        ...
                        child: Stack(
                          ...
                          children: [
                            Transform.rotate(
                              angle: compassRadianes,
                              child: Stack(
                                alignment: Alignment.center,
                                children: [
                                  Container(width: MediaQuery.of(context).size.width * 0.6, height: MediaQuery.of(context).size.width * 0.6, decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(color: Colors.greenAccent.withOpacity(0.2), width: 1))),
                                  Container(width: MediaQuery.of(context).size.width * 0.3, height: MediaQuery.of(context).size.width * 0.3, decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(color: Colors.greenAccent.withOpacity(0.2), width: 1))),
                                  const Positioned(top: 10, child: Text('N', style: TextStyle(color: Colors.red, fontWeight: FontWeight.bold))),
                                  const Positioned(bottom: 10, child: Text('S', style: TextStyle(color: Colors.grey, fontWeight: FontWeight.bold))),
                                  const Positioned(right: 10, child: Text('E', style: TextStyle(color: Colors.grey, fontWeight: FontWeight.bold))),
                                  const Positioned(left: 10, child: Text('O', style: TextStyle(color: Colors.grey, fontWeight: FontWeight.bold))),
                                ],
                              ),
                            ),
                            
                            Container(
                              decoration: BoxDecoration(shape: BoxShape.circle, boxShadow: [BoxShadow(color: Colors.greenAccent.withOpacity(0.4), blurRadius: 20, spreadRadius: 5)]),
                              child: CircleAvatar(
                                radius: 30,
                                backgroundColor: Colors.greenAccent,
                                backgroundImage: miPerfil['foto_url'] != null ? NetworkImage(miPerfil['foto_url']) : null,
                                child: miPerfil['foto_url'] == null ? const Icon(Icons.person, color: Colors.black, size: 30) : null,
                              ),
                            ),
                            ...
                            if (widget.miLatitud != null && widget.miLongitud != null)
                              ...perfilesCompatibles.map((perfil) {
                                ...
                                if (latOtro == null || lonOtro == null) return const SizedBox.shrink();

                                final distMetros = Geolocator.distanceBetween(widget.miLatitud!, widget.miLongitud!, latOtro, lonOtro);
                                final bearing = Geolocator.bearingBetween(widget.miLatitud!, widget.miLongitud!, latOtro, lonOtro);
                                
                                final direccionReal = bearing - heading; 
                                final anguloRadianes = (direccionReal - 90) * (math.pi / 180);
                                
                                final maxRadioPantalla = MediaQuery.of(context).size.width * 0.40;
                                final radioCalculado = (distMetros / (_distanciaMaximaKm * 1000)) * maxRadioPantalla; 
                                final radioVisual = radioCalculado.clamp(40.0, maxRadioPantalla); 
                                
                                final dx = radioVisual * math.cos(anguloRadianes);
                                final dy = radioVisual * math.sin(anguloRadianes);
                                ...
                                return Transform.translate(
                                  offset: Offset(dx, dy),
                                  child: GestureDetector(
                                    onTap: () => enviarSolicitud(context, perfil['id']),
                                    child: Column(
                                      mainAxisSize: MainAxisSize.min,
                                      children: [
                                        Stack(
                                        ...
                                        Container(
                                          ...
                                          decoration: BoxDecoration(color: Colors.black87, borderRadius: BorderRadius.circular(5), border: Border.all(color: Colors.greenAccent, width: 0.5)),
                                          child: Text(perfil['nombre'] ?? 'Explorador', style: const TextStyle(fontSize: 10, color: Colors.white, fontWeight: FontWeight.bold)),
                                        ),
                                        Text('${(distMetros / 1000).toStringAsFixed(1)} km', style: const TextStyle(fontSize: 9, color: Colors.orangeAccent)),
                                      ],
                                    ),
                                  ),
                                );
        ...
        builder: (context, snapshotSolicitudes) {
          ...
          return StreamBuilder<List<Map<String, dynamic>>>(
            ...
            builder: (context, snapshotMensajes) {
              ...
              for (var s in solicitudesTotales.where((s) => s['receptor_id'] == miId && s['estado'] == 'pendiente')) { pendientesUnicas[s['emisor_id']] = s; }
                        ...
                        ...pendientesUnicas.values.map((s) {
                          ...
                          return Card(
                            ...
                            child: ListTile(
                              ...
                              title: Row(children: [Text(emisor['nombre'] ?? 'Explorador', style: const TextStyle(color: Colors.white)), if (esVerificado) const Padding(padding: EdgeInsets.only(left: 4), child: Icon(Icons.verified, color: Colors.blueAccent, size: 16))]),
                        ...
                        return Card(
                          ...
                          child: ListTile(
                            leading: Stack(
                              clipBehavior: Clip.none,
                              children: [
                                GestureDetector(onTap: () => _mostrarPerfilRapidoLectura(context, otroPerfil), child: CircleAvatar(backgroundImage: fotoUrl != null ? NetworkImage(fotoUrl) : null, child: fotoUrl == null ? const Icon(Icons.person) : null)),
                                if (mensajesSinLeer > 0) Positioned(right: -6, top: -6, child: Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: Colors.red, shape: BoxShape.circle, border: Border.all(color: Colors.grey[900]!, width: 2)), child: Text('$mensajesSinLeer', style: const TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold))))
                              ],
                            ),
                            title: GestureDetector(
                               onTap: () => _mostrarPerfilRapidoLectura(context, otroPerfil),
                               child: Row(children: [Text(otroPerfil['nombre'] ?? 'Explorador', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)), if (esVerificado) const Padding(padding: EdgeInsets.only(left: 4), child: Icon(Icons.verified, color: Colors.blueAccent, size: 16))]),
                            ),
                            subtitle: matchMutuo ? const Text('❤️ Se gustan!', style: TextStyle(color: Colors.redAccent, fontWeight: FontWeight.bold)) : const Text('Toca aquí para abrir el chat'),
                            trailing: IconButton(icon: const Icon(Icons.delete_forever, color: Colors.redAccent), onPressed: () => _eliminarVinculoYCreados(context, s['id'].toString(), otroId, miId!)),
                            onTap: () { Navigator.of(context).push(MaterialPageRoute(builder: (_) => PantallaChat(receptorId: otroId, receptorNombre: otroPerfil['nombre'] ?? 'Explorador', receptorFoto: fotoUrl, solicitudId: s['id'].toString()))); },
                          ),
                        );
                      }),
                    ],
...
class _PantallaChatState extends State<PantallaChat> {
  ...
  Future<void> _subirArchivoMultimedia(ImageSource origen, String tipo) async {
    try {
      final XFile? archivo = tipo == 'video' ? await _picker.pickVideo(source: origen) : await _picker.pickImage(source: origen, imageQuality: 70);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Row(
          children: [
            CircleAvatar(radius: 18, backgroundImage: widget.receptorFoto != null ? NetworkImage(widget.receptorFoto!) : null, child: widget.receptorFoto == null ? const Icon(Icons.person, size: 20) : null),
            const SizedBox(width: 10),
          ...
          if (widget.solicitudId.isNotEmpty)
            StreamBuilder<List<Map<String, dynamic>>>(
              ...
              builder: (context, snapshot) {
                ...
                return Row(
                  children: [
                    if (match) const Padding(padding: EdgeInsets.only(right: 8.0), child: Text('❤️ Match Mutuo', style: TextStyle(color: Colors.redAccent, fontWeight: FontWeight.bold, fontSize: 12))),
                    IconButton(icon: Icon(yoDiLike ? Icons.favorite : Icons.favorite_border, color: Colors.redAccent), onPressed: () async { final campo = soyEmisor ? 'emisor_like' : 'receptor_like'; await Supabase.instance.client.from('solicitudes').update({campo: !yoDiLike}).eq('id', widget.solicitudId); }),
                  ],
                );
              }
            )
                ...
                child: StreamBuilder<List<Map<String, dynamic>>>(
                  ...
                  builder: (context, snapshot) {
                    ...
                    if (mensajesNoLeidos.isNotEmpty) {
                      WidgetsBinding.instance.addPostFrameCallback((_) { Supabase.instance.client.from('mensajes').update({'leido': true}).eq('receptor_id', miId!).eq('emisor_id', widget.receptorId).eq('leido', false).then((_) {}).catchError((_) {}); });
                    }
                    return ListView.builder(
                      controller: _scrollController, padding: const EdgeInsets.all(16), itemCount: mensajes.length,
                      itemBuilder: (context, index) {
                        final mensaje = mensajes[index];
                        final esMio = mensaje['emisor_id'] == miId;
                        final textoOriginal = mensaje['contenido'] ?? '';
                        DateTime fecha = DateTime.now();
                        if (mensaje['created_at'] != null) { fecha = DateTime.tryParse(mensaje['created_at'].toString())?.toLocal() ?? DateTime.now(); }
                        ...
                        if (textoOriginal.startsWith('[IMG]') && textoOriginal.length > 5) {
                          contenidoBurbuja = ClipRRect(borderRadius: BorderRadius.circular(8), child: Image.network(textoOriginal.substring(5), width: 200, fit: BoxFit.cover, errorBuilder: (context, error, stackTrace) => Container(width: 200, height: 100, color: Colors.grey[850], child: const Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(Icons.broken_image, color: Colors.grey, size: 30), SizedBox(height: 5), Text('Bloqueado', style: TextStyle(color: Colors.grey, fontSize: 10))]))));
                        ...
                        return Align(
                          ...
                          child: Container(
                            margin: const EdgeInsets.symmetric(vertical: 4), padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: esMio ? Colors.green[800] : Colors.grey[800], borderRadius: BorderRadius.circular(12)),
                            child: Column(
                              crossAxisAlignment: esMio ? CrossAxisAlignment.end : CrossAxisAlignment.start,
                              children: [
                                contenidoBurbuja, const SizedBox(height: 4),
                                Row(mainAxisSize: MainAxisSize.min, children: [Text(horaStr, style: const TextStyle(fontSize: 10, color: Colors.white70)), if (esMio) ...[const SizedBox(width: 4), Icon(Icons.done_all, size: 14, color: leido ? Colors.blueAccent : Colors.grey)]])
                              ],
                            ),
                          ),
                        );
              ...
              Padding(
                ...
                child: Row(
                  children: [
                    ...
                    _subiendoMedia ? const Padding(padding: EdgeInsets.all(12), child: SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2))) : IconButton(icon: const Icon(Icons.send, color: Colors.greenAccent), onPressed: () => enviarMensaje()),
                  ],
                ),
              ),
            ],
...
class _ReproductorVideoWidgetState extends State<ReproductorVideoWidget> {
  ...
  void initState() {
    ...
    _controller = VideoPlayerController.networkUrl(Uri.parse(widget.url))
      ..initialize().then((_) { if (mounted) setState(() => _inicializado = true); }).catchError((_) {});
  }
  ...
  @override
  Widget build(BuildContext context) {
    return _inicializado ? SizedBox(width: 220, child: AspectRatio(aspectRatio: _controller.value.aspectRatio, child: Stack(alignment: Alignment.center, children: [VideoPlayer(_controller), IconButton(icon: Icon(_controller.value.isPlaying ? Icons.pause_circle_filled : Icons.play_circle_filled, size: 50, color: Colors.white70), onPressed: () => setState(() => _controller.value.isPlaying ? _controller.pause() : _controller.play()))]))) : const SizedBox(width: 220, height: 150, child: Center(child: CircularProgressIndicator()));
  }
}